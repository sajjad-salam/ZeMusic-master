...
packages(){
	...
	if ! command -v ffmpeg &>/dev/null; then
		...
		if sudo apt install ffmpeg -y &>/dev/null;then
			...
			pprint "FAIL.\n\n" "cred"
			pprint "You need to install ffmpeg manually in order to deploy ZeMusic, exiting...\n" "cblue"
			exit 1
		fi
	fi
...
clear
pprint "Welcome to ZeMusic Setup Installer\n\n"
...
node
installation
pprint "\n\n\n\n\nZeMusic Installation Completed !" "cgreen"
sleep 1
clear
...
pprint "\n\n\nThanks for using ZeMusic installer, your vars have been saved successfully ! \nIf you wanna add more variables add them in your env by : vi .env"
